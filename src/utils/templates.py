...
async def list_templates(session: Session) -> list[TableTemplate]:
    templates = session.exec(select(TableTemplate)).all()
    return templates


async def create_template(template: Template, session: Session, user_id: int | None = None) -> TableTemplate:
...
async def get_user_templates(user_id: int, session: Session) -> list[TableTemplate]:
...
async def get_template(id: int, session: Session) -> TableTemplate:
    return session.get(TableTemplate, id)


async def delete_template(id: int, session: Session) -> bool:
