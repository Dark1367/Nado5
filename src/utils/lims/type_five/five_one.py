...
async def factorial(max_constant=5, min_constant=-5):
...
async def polynomial(count, max_constant=5, min_constant=-5):
    max_const = min_constant-1
    max_const_sign = "-"
    string = ""
    for i in range(count):
        sign = rand.random_choice(["+", "-"])
        s, constant = await factorial(max_constant, min_constant)
        if i == 0 and sign == "+":
            string += s
        else:
            string += sign + s
...
async def generate_lim():
    devisor, devisor_max_constant, devisor_max_const_sign = await polynomial(rand.randint(1, 3))
    denominator, denominator_max_constant, denominator_max_const_sign = await polynomial(rand.randint(1, 3))
